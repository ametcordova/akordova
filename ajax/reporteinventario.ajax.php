...
if(isset( $_GET["almacenInv"])){
    ...
    foreach ($respuesta as $key => $value) {
		...
		$importeExist=number_format($value["cant"]*$value["precio_venta"],2);
		$data[]=array(
		    ...
		    "7"=>'<td style="width:70px; text-align: center; background-color: yellow; color:red;">'.$value["stock"].'</td>',
 			"8"=>$resurtir,
 			"9"=>'<td style="text-align:right; width:80px;">'.$value["precio_venta"].'</td>',
 			"10"=>'<td style="text-align:right; width:80px;">'.$importeExist.'</td>',
        );        
    }
