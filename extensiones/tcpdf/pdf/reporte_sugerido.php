...
$bloque2 = <<<EOF
    ...
    <h3 style="text-align:center; width:800px">Sugerido de Compra de Almacen $tabla al $fechahoy</h3>
...
$bloque3 = <<<EOF
 	<table style="font-size:10px; padding:5px 5px;">
	  <tr bgcolor="#cccccc" class="text-center">
		...
		<td style="border: 1px solid #666; width:45px; text-align:center">Existen</td>
		<td style="border: 1px solid #666; width:45px; text-align:center">Máximo</td>
		<td style="border: 1px solid #666; width:50px; text-align:center">Resurtir</td>
        <td style="border: 1px solid #666; width:55px; text-align:center">P.Unit</td>
        <td style="border: 1px solid #666; width:40px; text-align:center">Caja</td>
		<td style="border: 1px solid #666; width:60px; text-align:center">Tot.Cto</td>
     </tr>
	</table>
EOF;
...
// ---------------------------------------------------------
$cantExiste=$cantMaximo=$cantSugerido=$TotalCosto=$sumatotalcajas=$TotalCajas=0;
foreach ($respuestaInventario as $row) {
    $sumatotalcajas=$sumatotalcto=0;
    if($row['surtir']<0){
        $sumacosto=number_format(abs($row["surtir"])*$row["precio_compra"],2,".",",");
        $preciocosto=number_format($row['precio_compra'],2,".",",");
        $descripcion=substr($row['descripcion'],0,34);
        $resurtir=abs($row['surtir']);
        $apedir=$cajas=$entero=$decimal=0;
        if($row['unidadxcaja']>0){
            $cajas=abs($resurtir/$row['unidadxcaja']);
            @list($entero,$fraccion) = explode('.',$cajas); 
            //$digito=$fraccion[0];       //obtener el primer digito de fraccion
            $digito=empty($fraccion[0])?0:$fraccion[0];       //obtener el primer digito de fraccion
            

            if(floor($digito)>8 and $entero==0){
                $apedir=1;
                $sumacosto=number_format($row['unidadxcaja']*$row["precio_compra"],2,".",",");
                // $sumatotalcto=$row['unidadxcaja']*$row["precio_compra"];
                $sumatotalcajas=($apedir*$row['unidadxcaja'])*$row["precio_compra"];
            }else if(floor($digito)>8 and floor($entero)>0){
                $apedir=$entero+1;
                $sumacosto=number_format(($apedir*$row['unidadxcaja'])*$row["precio_compra"],2,".",",");
                //$sumatotalcto=($apedir*$row['unidadxcaja'])*$row["precio_compra"];
                $sumatotalcajas=($apedir*$row['unidadxcaja'])*$row["precio_compra"];
            }else if(floor($digito)<9 and floor($entero)>0){
                $apedir=$entero;
                $sumacosto=number_format(($apedir*$row['unidadxcaja'])*$row["precio_compra"],2,".",",");
                //$sumatotalcto=($apedir*$row['unidadxcaja'])*$row["precio_compra"];
                $sumatotalcajas=($apedir*$row['unidadxcaja'])*$row["precio_compra"];
                ...
                $apedir="";
                $sumacosto=number_format(abs($row["surtir"])*$row["precio_compra"],2,".",",");
                $sumatotalcto=abs($row["surtir"])*$row["precio_compra"];
            }
...
$bloque4 = <<<EOF
         ...
         <table style="font-size:8px; padding:5px 3px;">
          ...
          <tr>
            ...
            <td style="border: 1px solid #666; width:45px; text-align:center">$row[cant]</td>
            <td style="border: 1px solid #666; width:45px; text-align:center">$row[stock]</td>
            <td style="border: 1px solid #666; width:50px; text-align:center">$resurtir</td>
            <td style="border: 1px solid #666; width:55px; text-align:right">$$preciocosto</td>
            <td style="border: 1px solid #666; width:40px; text-align:center">$apedir</td>
            <td style="border: 1px solid #666; width:60px; text-align:right">$$sumacosto</td>
          </tr>
...
EOF;
$cantExiste+=$row['cant'];
$cantMaximo+=$row['stock'];
$cantSugerido+=$row['surtir'];
$TotalCosto+=$sumatotalcto;
$TotalCajas+=$sumatotalcajas;
...
$bloque6 = <<<EOF
	...
	<tr bgcolor="#cccccc">
		<td style="border: 1px solid #666; width:478px; text-align:right">Totales:</td>
		<td style="border: 1px solid #666; width:45px; text-align:center">$cantExiste</td>
		<td style="border: 1px solid #666; width:45px; text-align:center">$cantMaximo</td>
		<td style="border: 1px solid #666; width:50px; text-align:center">$cantSugerido</td>
		<td style="border: 1px solid #666; width:95px; text-align:right">Total Costo</td>
		<td style="border: 1px solid #666; width:61px; text-align:right">$$TotalCosto</td>
    </tr>
    <tr bgcolor="#cccccc">
        <td style="border: 1px solid #666; width:713px; text-align:right">+ Tot. Costo Cajas</td>
        <td style="border: 1px solid #666; width:61px; text-align:right">$$TotalCajas</td>
    </tr>
 	</table>
...
<!-- 
<td style="border: 1px solid #666; width:55px; text-align:right">$$preciocosto</td>
            <td style="border: 1px solid #666; width:40px; text-align:center">$cajas</td>
            <td style="border: 1px solid #666; width:60px; text-align:right">$$sumacosto</td>
