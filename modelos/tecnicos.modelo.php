   ...
   /*=============================================
	...
	static public function mdlMostrarTecnicos($tabla, $item, $valor){
		...
		if($item != null){
			...
			$stmt = Conexion::conectar()->prepare("SELECT tec.id,tec.nombre, tec.expediente,tec.rfc,tec.telefonos, tec.direccion,tec.numero_licencia,alm.nombre AS almacen,tec.status FROM $tabla tec INNER JOIN almacenes alm ON alm_asignado=alm.id");
